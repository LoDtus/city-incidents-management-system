...
function NavigationBar() {
    ...
    return (
        <div ref={elementRef} className="navigationBar z-50 fixed h-[100vh] p-2 bg-white shadow-xl border-r border-gray-border">
            ...
            <ul className="navigationBar h-[100%] overflow-y-auto">
                <Link href={`/${role}/map`} title={t("navigation.map")}
                    className={curPage.includes("/map")
                        ? "item-navigation w-full py-2 px-2 mb-1 rounded-md flex items-center font-semibold text-blue bg-light-blue duration-200 hover:cursor-pointer"
                        : "item-navigation w-full py-2 px-2 mb-1 rounded-md flex items-center font-semibold text-gray duration-200 hover:cursor-pointer hover:text-black hover:bg-light-gray active:scale-90"}
                >
